...
const firebaseConfig = {
  ...
  databaseURL:
    "https://aitestbook-default-rtdb.asia-southeast1.firebasedatabase.app/", // <<--- यह Realtime Database के लिए महत्वपूर्ण है!
...
const navigationButtonsContainer =
  document.getElementById("navigation-buttons"); // [Source: 7]

const questionNavigationTable = document.getElementById(
...
const liveIncorrectAnswersSpan = document.getElementById(
...
async function loadQuestions(unitId) {
  ...
  if (!unitId) {
    quizContainer.innerHTML =
      "<p>कोई यूनिट चयनित नहीं है। कृपया होम पेज पर वापस जाएं और एक यूनिट चुनें।</p>";
    return; // [Source: 14]
  }
  ...
  try {
    ...
    if (
      ...
      quizContainer.innerHTML =
        "<p>इस यूनिट में कोई प्रश्न उपलब्ध नहीं है या सही फॉर्मेट में नहीं हैं।</p>";
      return; // [Source: 18]
    }
    ...
    if (navigationButtonsContainer)
      navigationButtonsContainer.style.display = "block"; // [Source: 23]
  ...
  currentQuestions.forEach((q, index) => {
    ...
    if (q && typeof q.question === "string" && Array.isArray(q.options)) {
      ...
      q.options.forEach((option, i) => {
        // [Source: 28]
        const optionValue =
          typeof option === "object" && option !== null ? option.text : option; // [Source: 28]
...
function handleOptionSelection(questionIndex, selectedRadio) {
  ...
  const allRadioButtonsInQuestion = optionsContainer.querySelectorAll(
    `input[name="question${questionIndex}"]`
  ); // [Source: 35]
...
function displayQuestion(index) {
  ...
  if (userAnswers[index] !== null) {
    ...
    if (selectedRadioButton) {
      ...
      const allRadioButtonsInQuestion = document.querySelectorAll(
...
function updateNavigationButtons() {
  ...
  if (backButton)
    backButton.style.display =
      currentQuestionIndex > 0 ? "inline-block" : "none"; // [Source: 39]
  if (nextButton)
...
function buildQuestionNavigation() {
  ...
  currentQuestions.forEach((_, index) => {
    // [Source: 42]
    navHtml += `<a href="#" data-qindex="${index}" id="qnav-${index}">${
      index + 1
    }</a>`; // [Source: 42]
  });
...
function updateLiveFeedback() {
  ...
  if (liveIncorrectAnswersSpan)
    liveIncorrectAnswersSpan.textContent = tempIncorrect; // [Source: 56]
}
...
if (clearOptionButton)
  clearOptionButton.addEventListener("click", () => {
    ...
    if (userAnswers[currentQuestionIndex] === null) {
      // केवल तभी साफ़ करें यदि कोई उत्तर चयनित नहीं (या पहले से साफ़ है)
      const radioButtons = document.querySelectorAll(
      ...
      const currentLabels = document.querySelectorAll(
        `#question-${currentQuestionIndex} .options label`
      ); // [Source: 58]
      ...
      const correctAnswerDisplay = document.getElementById(
        `correct-answer-${currentQuestionIndex}`
      ); // [Source: 58]
      if (correctAnswerDisplay) {
        // [Source: 58]
...
if (submitButtonSidebar)
  submitButtonSidebar.addEventListener("click", () => {
    // [Source: 59]
    const totalScore = calculateScore(); // [Source: 59]
    if (scoreElement)
      scoreElement.textContent = `${totalScore} / ${currentQuestions.length}`; // [Source: 59]
    ...
    if (navigationButtonsContainer)
      navigationButtonsContainer.style.display = "none"; // [Source: 59]
    ...
    document
      .querySelectorAll("#question-navigation-table a")
      .forEach((link) => (link.style.pointerEvents = "none")); // [Source: 59]
    ...
    currentQuestions.forEach((q, index) => {
      ...
      correctAnswerElement.innerHTML = `<b>सही उत्तर:</b> ${
        q.answer || "उपलब्ध नहीं"
      }`; // [Source: 63]
      ...
      if (optionsContainer) {
        ...
        const caDisplay = questionCard.querySelector(
          `#correct-answer-${index}`
        );
      ...
      const radioInputs = questionCard.querySelectorAll(
        `input[name="question${index}"]`
      ); // [Source: 67]
...
if (increaseFontButton)
  increaseFontButton.addEventListener("click", () => {
    ...
    if (baseFontSize < 24) {
      //
      baseFontSize += 2; //
      updateFontSize(); // नया फ़ंक्शन कॉल करें
    }
  });
...
function updateFontSize() {
  if (quizContent) quizContent.style.fontSize = `${baseFontSize}px`;
  if (currentFontSizeSpan)
    currentFontSizeSpan.textContent = `${baseFontSize}px`;

  // प्रश्न टेक्स्ट और विकल्पों को अपडेट करें
  document
    .querySelectorAll(".question-card p, .options label span")
    .forEach((element) => {
...
if (shuffleQuestionsButton)
  shuffleQuestionsButton.addEventListener("click", () => {
    // [Source: 75]
    if (
      confirm(
        "क्या आप प्रश्नों को शफल करना चाहते हैं? आपकी वर्तमान प्रगति रीसेट हो जाएगी।"
      )
    ) {
      // [Source: 75]
...
if (shuffleOptionsButton)
  shuffleOptionsButton.addEventListener("click", () => {
    // [Source: 76]
    if (
      confirm(
        "क्या आप विकल्पों को शफल करना चाहते हैं? आपकी वर्तमान प्रगति रीसेट हो जाएगी।"
      )
    ) {
      // [Source: 76]
...
function resetQuizStateAndReload() {
  ...
  if (quizContainer)
    quizContainer.innerHTML = "<p>प्रश्न पुनः लोड हो रहे हैं...</p>"; // [Source: 77]
  ...
  if (navigationButtonsContainer)
    navigationButtonsContainer.style.display = "block"; // [Source: 79]
...
window.onload = () => {
  ...
  updateFontSize(); // प्रारंभिक फ़ॉन्ट साइज़ सेट करें
